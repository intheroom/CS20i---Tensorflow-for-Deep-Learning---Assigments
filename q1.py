...
x = tf.constant([29.05088806,  27.61298943,  31.19073486,  29.35532951,
 ...
 27.88236427,  20.56035233,  30.20379066,  29.51215172,
 33.71149445,  28.59134293,  36.05556488,  28.66994858])
out = tf.gather(x, tf.where(tf.greater(x, 30.0)))
print(sess.run(out))
...
out = tf.cond(tf.less(tf.reduce_mean(x - y), 0), lambda: tf.reduce_mean(tf.square(x - y)), lambda: tf.reduce_sum(tf.abs(x - y)))
print(sess.run(out))
